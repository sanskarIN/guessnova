...
4. Run lint, format, strict mypy, tests, compile, release-metadata verification, smoke, entry-point/both-Textual-import checks, dependency audit, build, and Twine checks.
...
11. Verify v1.5 Challenge Setup: Classic/Timed/Streak/Daily selection, shared difficulty choices, non-Daily integer seed, Daily ISO date, mode-aware field disabling, target-free identity, and Reverse exclusion from numeric setup.
12. Verify invalid challenge seed/date input leaves the active target/attempt state intact and focuses the relevant field.
13. Verify configured seeded/Daily reset reconstructs the same deterministic challenge and returns focus to Guess.
14. Verify profile-switch unfinished-round isolation, exact-name recoverable deletion, history/leaderboard filtering, settings persistence, high contrast, and read-only backup verification.
15. Complete [`accessibility_evidence_template.md`](accessibility_evidence_template.md) on the exact signed-off release candidate; v1.5 evidence includes Challenge Setup plus all six panes.
16. Verify English and Hindi presentation, including Challenge Setup and a TUI relaunch after changing profile locale.
17. Create immutable semantic tag `v1.5.0` only for project version `1.5.0` after all required automated and manual gates pass.
18. Push the tag; the release workflow independently reruns strict and cross-platform release gates before creating artifacts.
19. Verify installation from the built wheel in a clean Python 3.13 environment and confirm `guessnova`, `guessnova-tui`, `guessnova-doctor`, `guessnova doctor` routing, stable workspace import, and shipped challenge-app import are present.
20. Add real screenshots/demo media only when captured from the exact signed-off build according to [`media/README.md`](media/README.md).
